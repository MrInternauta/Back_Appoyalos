...
import gm from 'gm';
import { getUserByIdOrEmail, updateUser } from '../controllers/usuariosController';
...
export async function borraArchivo(nombreImagen, tipo = 'usuario') {
  let pathImagen = path.resolve(__dirname, `${process.env.IMAGES_PATH}/${tipo}/${nombreImagen}`);
  if (fs.existsSync(pathImagen)) {
