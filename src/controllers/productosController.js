...
export async function getAllByCategoria(req, res) {
  try {
    ...
    let productos = await SEQUELIZE_CONFIG.query(
      'SELECT P.* FROM productos as P LEFT JOIN categorias as C on P.idcategorias = C.idcategorias WHERE c.idcategorias =  :parameter',
      {
        replacements: {
...
export async function getById(idProducto) {
  try {
    return await Productos.findOne({ where: { idproductos: idProducto } });
  } catch (error) {
    return error;
  }
}
...
export async function findProducts(req, res) {
  try {
    ...
    parameter = '%' + parameter + '%';
    let productos = await SEQUELIZE_CONFIG.query(
      'SELECT P.* FROM productos as P inner join categorias as C on P.idcategorias = C.idcategorias WHERE  P.nombre LIKE :parameter  OR P.descripcion LIKE :parameter  OR C.nombre LIKE :parameter ',
      {
        replacements: {
...
export async function create(req, res) {
  let { nombre, descripcion, imagenurl, existencia, idcategorias, id_usuario } = req.body;
...
export async function update(req, res) {
  let { idproductos, nombre, descripcion, imagenurl, existencia, idcategorias, id_usuario, activo } = req.body;
