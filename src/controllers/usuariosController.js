  ...
  try {
    ...
    if (responseUser.status) {
      if (responseUser.data != null)
        return res
          .status(500)
          .json({ ok: false, message: "Error: La curp o el correo ya existe." });
    } 
    ...
    if (newUser) {
      ...
      let data = await CreateHtmlSignIn({
        usuario: newUser,
        url,
...
export async function remove(req, res) {
  try {
    ...
    const RESPONSE = {
      status: true,
      message:
        usuario <= 0 ? "No existe usuario" : "Usuario eliminado correctamente",
    };
...
export async function confirmarEmail(req, res) {
  try {
    ...
    let respuesta;
    let usuario = await SEQUELIZE_CONFIG.query(
      "SELECT * FROM usuarios WHERE correo = :correo OR  curp = :correo",
      {
        replacements: {
          correo: correo ? correo.toLowerCase() : "",
        },
